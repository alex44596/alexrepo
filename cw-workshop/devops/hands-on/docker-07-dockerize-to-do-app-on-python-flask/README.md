...
- Create a `to-do-api.py` with following coding. Special RDS DB instance configured within following code is already up and running for this project.
...
```bash
docker build -t <dockerhub-username>/todolist:latest .
```

- Run your docker image.

```bash
docker run --name todo -d -p 80:80 <dockerhub-username>/todolist
```
...
```bash
docker push <dockerhub-username>/todolist
```
